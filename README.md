...
> #### State를 업데이트할 때
> React에서 배열이나 객체 형태의 State를 업데이트할 때 <br/>
> 기존 State를 직접 수정하지 않고 새로운 State를 만들어 사용하는 이유: <br/>
> React는 State 객체/배열의 '참조'가 변경될 때 리렌더링을 결정. <br/>
> 새 객체/배열을 만들면 참조가 달라져 React가 변경을 감지하고 UI를 업데이트함. <br/>

> #### 배열 데이터의 map, key
> React에서 배열 데이터를 기반으로 <br/>
> 여러 컴포넌트나 엘리먼트 목록을 화면에 렌더링할 때 사용되는 배열 메소드와 <br/>
> 각 항목에 반드시 설정해야 하는 속성: <br/>
